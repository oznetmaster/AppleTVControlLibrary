...
/// <summary>
/// Server-side SRP session used by <see cref="FakeMrpDevice"/> to emulate the Apple TV's half of pair-setup.
/// </summary>
/// <remarks>
/// Identical algorithm to <c>AppleTvControlLibrary.FakeDevice.FakeCompanionSrpServer</c> (both are ported
/// from srptools' <c>SRPContext</c>/<c>SRPServerSession</c>); duplicated here rather than shared because the
/// Companion and MRP fake device projects are independent test-only assemblies.
/// </remarks>
// pyatv/protocols/mrp/server_auth.py (new_server_session) — line 41-63 as of pyatv 0.18.0;
// srptools/context.py, srptools/server.py, srptools/common.py (SRPServerSession)
public sealed class FakeMrpSrpServer
	{
	...
	private FakeMrpSrpServer (BigInteger serverPrivate, BigInteger passwordVerifier, byte[] salt, BigInteger serverPublic)
		{
	...
	public static FakeMrpSrpServer Create (int pin)
		{
		...
		return new FakeMrpSrpServer (serverPrivate, passwordVerifier, salt, serverPublic);
		}
