...
public class HapCredentialsTests
	...
	[Test]
	public void TransientCredentialsHaveTransientType () => Assert.That (HapCredentials.TransientCredentials.Type, Is.EqualTo (AuthenticationType.Transient));

	[Test]
	public void InvalidCombinationThrows () => _ = Assert.Catch<InvalidCredentialsException> (() => new HapCredentials (ltpk: [0x01]));
	}