...
namespace AppleTvControlLibrary.ScanTool;

/// <summary>
/// Standalone CLI that runs a real Companion Link mDNS scan against the local network and
/// prints every discovered device, so results can be verified against real Apple TV hardware
/// without spinning up Visual Studio's test runner.
/// </summary>
internal static class Program
	...
	private static async Task<int> Main (string[] args)
		...
		Console.WriteLine ($"Scanning for Companion Link devices (\"{CompanionServiceInfo.SERVICE_TYPE}\") for {timeout.TotalSeconds}s...");
		...
		IReadOnlyList<CompanionDiscoveryResult> results;
		try
			{
		...
		if (results.Count == 0)
			{
			Console.WriteLine ("No Companion Link devices found.");
			return 0;
			}
