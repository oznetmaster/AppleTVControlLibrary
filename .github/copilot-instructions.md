...
2. **Do not invent field names, type bytes, or enum values.** If a value cannot be found in the vendored pyatv tree, stop and ask. Do not infer it from the protocol documentation at pyatv.dev — that page is stale and contains known errors (§7). **This applies to this brief as well:** if a claim here cannot be confirmed against the vendored source, the source wins and the brief is wrong.
...
5. **A missing capability is a real answer.** If a feature has no representation in the source, the correct outcome is to expose it as unsupported — not to approximate it, and not to assume a later pyatv version added it. Ask before building a workaround.
...
>
> A previous revision of this brief claimed the `_hidC` enum "has grown past 19 values (Guide and Control Center were added in 0.17.0)." That was fabricated — the enum has exactly 19 values and there is no Control Center — and it sent an agent hunting for values that do not exist. Rule 2 now explicitly binds this document too.
...
**Multi-target the test project as well.** Byte-exactness is the whole deliverable, and the `net472` leg runs different `Span<T>`, `BitConverter`, and encoding implementations underneath. A suite that only runs on `net10.0` leaves the polyfill path unexercised — which is exactly where a divergence would hide.
...
### WP6 — Protocol dispatch and session lifecycle
...
**`HidCommand` — `api.py:35-56`. Exactly these nineteen values. There are no others in 0.18.0.**
...
There is no mute, no Control Center, no "back". Do not extrapolate values past 19 — the enum ends there. `grep -rni mute pyatv/protocols/companion/` returns nothing at all across the whole protocol; see §4 for how mute is actually built.

Wire form (`api.py:305-309`): `_hidC` with `{"_hBtS": 1 if down else 2, "_hidC": <value>}`. A press is two commands, down then up.

**Touch** (`api.py:311-330`, `const.py:460-466`): `TouchAction` is `Press = 1, Hold = 3, Release = 4, Click = 5` — note 2 is unused. Coordinates are clamped to `TOUCHPAD_WIDTH = 1000.0` and `TOUCHPAD_HEIGHT = 1000.0` (`api.py:88-89`), and `TOUCHPAD_DELAY_MS = 16` (`api.py:90`) is the inter-event delay for a swipe. Events also carry `_ns`, a nanosecond timestamp.

### WP8 — Media control and power
...
**`MediaControlCommand` — `api.py:59-74`:**
```
...
Wire form (`api.py:395-399`): `_mcc` with `{"_mcc": <value>, …args}`.

Arguments confirmed in source:
- `SetVolume` ? `{"_vol": <0.0–1.0>}` (`__init__.py:469-471`)
- `GetVolume` ? response at `_c._vol`, also `0.0–1.0` (`__init__.py:443-444`)
...
**Capability gating — `MediaControlFlags`, `__init__.py:87-101`:**
```
...
| Volume commands accepted, nothing happens | `_mcF & 0x0100` clear — volume is on CEC, not Companion |
...
Everything in this brief is `pyatv 0.18.0`. If you upgrade the vendored tree, re-check in this order — these are the things that have actually moved between releases:

1. `HidCommand` — values may be appended
2. `MediaControlCommand` and `MediaControlFlags` — flags 0x0040 and 0x0080 are currently unidentified
